...
describe('signup', () => {
  ...
  it('redirect balik ke /signup dengan pesan error kalau signUp gagal', async () => {
    signUpMock.mockResolvedValue({ error: { message: 'Email sudah terdaftar' } })
    const formData = buildFormData({
      email: 'dup@test.com',
      password: 'rahasia123',
      name: 'Budi',
...
describe('login', () => {
  it('redirect ke /sop kalau login berhasil', async () => {
    ...
    const formData = buildFormData({ email: 'user@test.com', password: 'rahasia123' })
  ...
  it('redirect balik ke /login dengan pesan error kalau kredensial salah', async () => {
    signInWithPasswordMock.mockResolvedValue({ error: { message: 'Email atau password salah' } })
    const formData = buildFormData({ email: 'user@test.com', password: 'salah' })
